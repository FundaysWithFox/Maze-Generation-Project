...
if cells_visited < maze_width * maze_height
{
	...
	if y_pos > 0
	{
		if !maze_status[array_get_coord(x_pos, y_pos - 1, maze_width)][cell.CELL_VISITED]
		{
	...
	{
		if !maze_status[array_get_coord(x_pos + 1, y_pos, maze_width)][cell.CELL_VISITED]
		{
	...
	else
	{
		...
		while found = false
		{
			...
			{
				if maze_status[pointer_pos - maze_width][cell.CELL_VISITED]
				{
					ds_list_add(neighbors, cell.CELL_PATH_NORTH);
				}
			}
			...
			{
				if maze_status[pointer_pos + 1][cell.CELL_VISITED]
				{
					ds_list_add(neighbors, cell.CELL_PATH_EAST);
				}
			}
			...
			if !maze_status[pointer_pos][cell.CELL_VISITED] && !ds_list_empty(neighbors)
			{
				...
				maze_status[pointer_pos][cell.CELL_VISITED] = true;
				cells_visited++;
				found = true;
			}
			else
			{
				if maze_status[pointer_pos][cell.CELL_VISITED] && currently_clear cells_clear++;
